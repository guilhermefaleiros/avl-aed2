...
public class ArvoreAVL {
    ...
    public void adicionarNaListaEmOrdem(List<Elemento> elementos) {
        if (!isEmpty()) {
            if (this.esq != null) {
                this.esq.adicionarNaListaEmOrdem(elementos);
            }
            elementos.add(this.ele);
            if (this.dir != null) {
                this.dir.adicionarNaListaEmOrdem(elementos);
            }
        }
    }
    ...
    public void calcularElementosComUmFilho() {
        if ((this.esq != null && this.dir == null) || (this.dir != null && this.esq == null)) {
            ArvoreApp.SUM++;
        }
        if (this.dir != null) {
            this.dir.calcularElementosComUmFilho();
        }
        if (this.esq != null) {
            this.esq.calcularElementosComUmFilho();
        }
    }
    ...
    public String consultaIntervalar(int x, int limInf, int limSup) {
        if (isEmpty()) {
            return null;
        }
        if (this.ele.getValor() == x) {
            return "O valor " + x + " se encontra na AVL.";
        } else {
            if (this.ele.getValor() > x && limSup > this.ele.getValor()) {
                limSup = this.ele.getValor();
            }
            if (this.ele.getValor() < x && limInf < this.ele.getValor()) {
                limInf = this.ele.getValor();
            }
            if (x < this.ele.getValor()) {
                if (this.esq == null) {
                    return "Limite inferior de x encontrado na AVL: " + limInf + " \n" + "Limite superior de x encontrado na AVL: " + limSup;
                } else {
                    return this.esq.consultaIntervalar(x, limInf, limSup);
                }
            } else if (x > this.ele.getValor()) {
                if (this.ele.getValor() > x && limSup > this.ele.getValor()) {
                    limSup = this.ele.getValor();
                }
                if (this.dir == null) {
                    ...
                    return this.dir.consultaIntervalar(x, limInf, limSup);
                }
            }
            return null;
        }
